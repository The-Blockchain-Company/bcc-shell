...
\title{Specification of the Bcc shell}
...
\begin{abstract}
This document is a high-level specification of how the pieces that make Bcc shell work together and an attempt to define a general, secure and simple way to combine them, while introducing the reader with the specifics of how they work.
...
Currently, there is an attempt to briefly cover the pieces of \textit{bcc-shell} functionality with TLA+.
The small amount of work that was done, in the interest of time, can be found here -  \url{https://github.com/The-Blockchain-Company/bcc-shell/tree/develop/specs/tla}.\\

There is a TLA+ model that I found clarified my understanding when trying to model the IPC protocol - \url{https://github.com/The-Blockchain-Company/bcc-shell/blob/develop/specs/tla/IPCProtocol.pdf}.\\

The other parts are just experiments waiting for completion.\\

For example, this is what I started to do to check how the update system will work, but realized that it would have to be really quite more involved so I stopped at that point (in the interest of time) - \url{https://github.com/The-Blockchain-Company/bcc-shell/blob/develop/specs/tla/UpdateSystem.pdf}.

\newpage
\section{IPC and communication between Klarity and Node}
\label{sec:ipc}

Currently, the \textit{Klarity} and the \textit{Node} (this is what I'm calling the node, thus the uppercase) communicate via \textbf{IPC for reaching a consensus which port to use for the JSON API communication, since we have a lot of issues of ports being used} .
...
When the \textit{Klarity} calls and starts the \textit{Node}, it also opens up the \textbf{IPC} protocol to enable communication between the two.
First, the \textit{Node} sends the message \textbf{Started} back to the \textit{Klarity} to inform him that the communication can begin.
After that, \textit{Klarity} sends the message \textbf{QueryPort} to the \textit{Node}, and the \textit{Node} responds with the free port it found using \textbf{ReplyPort PORTNUM} that is going to be used for starting the HTTP "server" serving the \textit{JSON API} which they can then use to communicate further.\\
...
We can easily generalize this concept. We can say that \textit{Klarity} is the \textit{Server}, and that the \textit{Node} is the \textit{Client}. Since the communication is bi-directional, we can say that either way, but we can presume that the \textit{Server} is the process which is started first.

What does this bring us? It brings us the ability to \textbf{decouple} our implementation from the specific setting where \textit{Klarity} is the \textit{Server} and the \textit{Node} is the \textit{Client}. There are some ideas that we might switch the order of the way we currently run these two, so we can freely implement that either way. For example, when we write tests for this communication protocol, we need to both start the server and the client and then check their interaction. It also removes any extra information that we might need to drop anyway. What we are focusing here is the \textbf{communication protocol} and not it's actors. We don't really care who says to whom, we are interested in seeing what is being said - \textit{how the whole protocol works}.
...
% https://github.com/The-Blockchain-Company/fm-ledger-rules/pull/108/files
...
Currently, the \textit{Klarity} and the \textit{Node} (this is what I'm calling the node, thus the uppercase) communicate via \textbf{JSON API} once they have settled in on a port via which to communicate (see \hyperref[sec:ipc]{here}).
First of all, we need to understand that the blocks in the blockchain contain the version of \textit{Klarity} (the frontend).
We can say that \textit{Klarity}, also known as \textit{frontend} is the \textit{Server}, and that the \textit{Node}, also known as \textit{backend} is the \textit{Client}, which are the same things under different names.
...
\begin{figure}[ht]
  ...
  \begin{sequencediagram}
    ...
    \newinst{cl}{Bcc launcher}{BccLauncher}
    \newinst{da}{Klarity}{Klarity}
    \newinst{cn}{Bcc node}{BccNode}
    ...
    \postlevel
    \mess{cl}{Starts the Klarity frontend with node arguments}{da}

    \postlevel
    \mess{da}{Starts the node with node arguments from Klarity}{cn}
...
The specifics of how this works are a bit tricky. We use the \textbf{bcc-launcher} also known simply as \textbf{Launcher} is something we require so we can have control over the (Electron) Klarity process and to be sure it shuts down correctly.

...
import           Bcc.Prelude hiding (onException)
...
import           Bcc.Shell.Configuration (WalletArguments (..),
                                              BccWalletBin (..))
...
import           Bcc.Shell.Update.Lib (RemoveArchiveAfterInstall (..),
                                           ...
                                           runUpdater)
import           Bcc.X509.Configuration (DirConfiguration (..), certChecks,
...
data BccWalletExitCode
    = RunUpdate
    -- ^ Klarity is ready to run the update system.
    | RestartInGPUSafeMode
    -- ^ Klarity asks launcher to relaunch Klarity with GPU safe mode.
    | RestartInGPUNormalMode
    -- ^ Klarity asks launcher to relaunch Klarity with normal mode (turn off GPU safe mode).
    | ExitCodeFailure Int
    -- ^ Klarity wants launcher to exit as well. Unexpected exit code.
    | ExitCodeSuccess
    -- ^ Klarity "happy path" where it could shut down with success.
...
instance Isomorphism BccWalletExitCode ExitCode where
...
handleBccWalletExitCode
    ...
    -> RestartRunner
    -> BccWalletExitCode
    -> IO BccWalletExitCode
handleBccWalletExitCode runUpdater' restartWallet = isoTo <<$>> \case
...
newtype WalletRunner = WalletRunner
    { runWalletSystemProcess :: BccWalletBin -> WalletArguments -> IO ExitCode
    }
...
runWalletProcess
    ...
    logNotice logDep $ "Starting the wallet with arguments: " <> Bcc.Prelude.show walletArgs

    -- create the wallet process
    walletExitStatus <- runWalletSystemProcess walletRunner bccwalletBin walletArgs

    logNotice logDep $ "Wallet exited with exitcode: " <> Bcc.Prelude.show walletExitStatus

    -- Let us map the interesting commands into a very simple "language".
    let exitCode :: BccWalletExitCode
    ...
    let msg = case exitCode of
            ...
            RestartInGPUNormalMode ->
                "Restart Klarity in normal mode"
            RestartInGPUSafeMode ->
                "Restart Klarity in GPU safe mode"
            ExitCodeSuccess ->
                "Klarity exited with ExitSuccess, will shutdown bcc-launcher"
            ExitCodeFailure af -> mconcat
                [ "Klarity exited with ExitFailure "
                , Bcc.Prelude.show af
                , ", will shutdown bcc-launcher"
                ]
    ...
    isoFrom <$> handleBccWalletExitCode
...
walletRunnerProcess = WalletRunner $ \bccwalletBin walletArgs ->
    system (createProc Process.Inherit bccwalletBin walletArgs) mempty
  where
    ...
    createProc stdStream (BccWalletBin commandPath) (WalletArguments commandArguments) =
...
runLauncher
    ...
    -> FilePath
    -> IO ExitCode
runLauncher loggingDependencies walletRunner bccwalletBin runUpdateFunc updaterData stateDir = do

...
import           Bcc.Shell.CLI (LauncherOptionPath (..),
                                    decodeLauncherOption, setupEnvVars)
import           Bcc.Shell.Configuration (LauncherOptions (..),
                                              setWorkingDirectory)
import           Bcc.Shell.Launcher (BccWalletExitCode (..), WalletMode (..),
...
launcherSystemSpec =
    describe "Launcher system" $ modifyMaxSuccess (const 10000) $ do
        ...
        it "should return success" $ monadicIO $ do
            ...
            exitCode <- run $ handleBccWalletExitCode doNotUse doNotUse walletExitCode
        ...
        it "should restart launcher, normal mode" $ monadicIO $ do
            ...
            exitCode <- run $ handleBccWalletExitCode doNotUse launcherFunction walletExitCode
            assert $ exitCode == ExitCodeSuccess
        ...
        it "should run update, restart launcher, normal mode" $ monadicIO $ do
            ...
            exitCode <- run $ handleBccWalletExitCode updateFunction launcherFunction walletExitCode
            assert $ exitCode == ExitCodeSuccess
...
-- These are config files downloaded from Klarity CI
--
...
withSetEnvs path homePath action =
  ...
  where
    setEnvs :: IO ()
    setEnvs = do
        -- Scripts that runs the launcher sets KLARITY_CONFIG variable
        setEnv "KLARITY_CONFIG" "Foo"
